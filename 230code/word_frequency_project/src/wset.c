 ...
 */
WordSet *wset_new () {
    WordSet* NewWordSet = (WordSet*)malloc(sizeof(WordSet));
    NewWordSet -> size =0;
  
    NewWordSet -> head = NULL;
...
static WordNode **search (WordSet *wset, char *word) {
  ...
  // put loop here
    for ( p = &(wset->head);(*p)!=NULL;p = &((*p)->next)){
 ...
 * the set.  In any case we increment its frequency. The function returns the
 * (new) frequency of the word.
 */
int wset_add (WordSet *wset, char *word) {
  ...
  // function.
  WordNode** searchedNode = search(wset,word);
    if((*searchedNode) == NULL){
        *searchedNode=node_new(word);
        ++(wset->size);
    }
  // The search function is designed to make this function relatively easy to
  // write!
  //
  // This function is used in the words.c program.
  return ++(*searchedNode)->freq;
}
...
int wset_fast_add (WordSet *wset, char *word) {
  ...
  // write!
   WordNode** searchedNode = search(wset,word);
    ...
    if (wset->head != (*searchedNode) ){
        WordNode* nextNode = (*searchedNode)->next;
        (*searchedNode)->next = wset->head;
        wset -> head = (*searchedNode);
        (*searchedNode)=nextNode;
        searchedNode=&(wset->head);
    }

  // This function is used by the words.c program.
    return ++(*searchedNode)->freq;
}
...
int wset_remove (WordSet *wset, char *word) {
  ...
  // function is designed to make this function easy to write!
    ...
    if ((*searchedNode)==NULL){
        ...
        WordNode* removedNode = (*searchedNode);
        
        int freqen = removedNode -> freq;
        
        *searchedNode = removedNode->next;
        
        node_free(removedNode);
        --(wset->size);
...
/**
 * The wset_exists function is used to test the existance of
 ...
 * in the WordSet object. If the word does not exist it returns -1.
 */
int wset_freq (WordSet *wset, char *word) {
  // TODO: search the for the word - return its frequency.  If you do not find
  // the word then return -1.
  WordNode* r = *search(wset, word);
    if (r==NULL){
        return -1;
  }
    else{
...
void wset_print (WordSet *wset) {
  ...
  int freqsize;
  for (freqsize = 0; maxfreq > 0; maxfreq /= 10, ++freqsize) ;
  for (WordNode *p = wset->head; p != NULL; p = p->next) {
    printf("%-*s : %*d\n", longest, p->word, freqsize, p->freq);
  }
}
