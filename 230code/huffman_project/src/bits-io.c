...
BitsIOFile *bits_io_open (const char *name, const char *mode) {
  ...
  bfile->bitsLeft = (mode_letter == 'w' ? 8 : 0);
                    ...
                    // and waiting for the byte to file when writing.
  bfile->byte     = 0;
  bfile->numBytes = 0;
  return bfile;
}
...
int bits_io_read_bit (BitsIOFile *bfile) {
  ...
  // read another byte from the file.  If that fails (returns -1),
  ...
  // the bit that we obtained.

    if(bfile->bitsLeft<=0){
        int result = fgetc(bfile->fp);
        if (bfile->bitsLeft<0){
            return EOF;
        }
        ++(bfile->numBytes);
