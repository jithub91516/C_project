...
TEST(ProjectTests, test_cache_initialization_set_bits)
{
  ...
  ASSERT_EQ(sets, cache->set_bits) << "cache->set_bits improperly initialized: expected " << sets;
}
...
TEST(ProjectTests, test_cache_initialization_block_bits)
{
  ...
  // Test the basic initialization of the cache
  ASSERT_EQ(bytes, cache->block_bits) << "cache->block_bits improperly initialized: expected " << bytes;
}
...
TEST(ProjectTests, test_cache_initialization_line_count)
{
  ...
  // Test the basic initialization of the cache
  ASSERT_EQ(lines, cache->line_count) << "cache->line_count improperly initialized: expected " << lines;
}
...
TEST(ProjectTests, test_cache_set_and_block_counts)
{
  ...
  ASSERT_EQ(set_count, cache->set_count) << "cache->set_count improperly initialized: expected " << set_count;

  ASSERT_EQ(block_size, cache->block_size) << "cache->block_size improperly initialized: expected " << block_size;
}
...
TEST(ProjectTests, test_cache_set_lists_line_count)
{
  ...
  ASSERT_TRUE(!testval) << "line_count in Set is properly initialized; expected " << lines;
}
...
TEST(ProjectTests, test_wc_trace)
{
  ...
  ASSERT_TRUE(cpu->conflict > 22038 &&
	      cpu->conflict < 22138) <<
